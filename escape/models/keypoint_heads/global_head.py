...
class GlobalHead(TopdownHeatmapBaseHead):
    ...
    def forward_train(self, img, target, target_weight, feature, alignment_loss, 
        ...
        """
        #print(keypoint_index_onehot.shape)torch.Size([32, 68, 965])

        #print(self.keypoints.unsqueeze(-1).unsqueeze(-1).shape) torch.Size([1093, 256, 1, 1])
        keypoints = torch.einsum('bks,schw->bkchw', keypoint_index_onehot,
                                 self.keypoints.unsqueeze(-1).unsqueeze(-1)[:keypoint_index_onehot.shape[-1],...])
        keypoints = keypoints.flatten(0, 1)
        batch_size = feature.size(0)
        feature = feature.reshape(1, -1, *feature.size()[2:])
        #print(feature.shape)
        ...
        #print(losses)
        losses["mse_loss"] +=  0.0004 * alignment_loss
    ...
    def forward_global_test(self, keypoint_index_onehot, **kwargs):
        keypoints = torch.einsum('bks,sc->bkc', keypoint_index_onehot,
                                 self.keypoints[:keypoint_index_onehot.shape[-1], ...  ])
        return keypoints
