...
from mmpose.models.builder import BACKBONES  # 用于加载注册的模型
from mmpose.core.post_processing import get_warp_matrix, warp_affine_joints
...
class TopDownAffineFewShot:
    def __init__(self, use_udp=False):
        ...
        # 使用注册名称加载 Mamba-UNet 作为 Backbone 模型
        #self.mamba_unet = BACKBONES.get('VMUNet')()  # 使用注册的名称来实例化
        #self.mamba_unet = self.mamba_unet.to(self.device)
    ...
    def __call__(self, results):
        ...
        # 使用 Mamba-UNet 模型提取特征
        #vmunet_input = torch.tensor(img).permute(2, 0, 1).unsqueeze(0).to(self.device).float() / 255.0  # 转换为 tensor
        #with torch.no_grad():
            #mamba_features = self.mamba_unet(vmunet_input)

        # 打印特征信息
        #self.print_features(clip_features, mamba_features)

        # 将特征添加到结果字典中
        #clip_features = torch.tensor(clip_features).unsqueeze(0)
        #clip_features = clip_features.to('cpu').numpy()
 
        #results['clip_features'] = clip_features
        #print("img shape:", img.shape)
        #results['vmunet_input'] = vmunet_input
...
class TopDownGenerateTargetFewShot:
    ...
    def __call__(self, results):
        ...
        #clip_feature = results['clip_features']
        #clip_feature = torch.tensor(clip_feature)
        #print("img_for_clip shape:", img_for_clip.shape)]
        #print(results['img'])  # torch (3,256,256)
        #print(f"clip feature shape: {clip_feature.shape}") #torch.Size([1, 1, 512])
        #results['clip'] = clip_feature
        #print(f"img is {results['img']}")
